...
function favoriteNumber(req, res) {
    ...
    Favorite.find({ "movieId": req.params.movieId })
        .exec((err, favorite) => {
...
function favorited(req, res) {
    ...
    Favorite.find({ "movieId": req.params.movieId, "userFrom": req.user._id })
        .exec((err, favorite) => {
            if (err) return res.status(400).send(err)
...
function removeFromFavorite (req, res) {
    ...
    Favorite.findOneAndDelete({ movieId: req.params.movieId, userFrom: req.user._id })
        .exec((err, doc) => {
